...
void p2p::Formation(void){
	m_discovery->remove(m_address, 20); // setDiscoverable(false)
	//Simulator::Schedule(MilliSeconds(0), &FormationAux, this, ++i);
	FormationAux(0, randomGen->GetInteger(5,12));
}

void p2p::FormationAux(uint32_t i, uint32_t limit){
	...
	if (distance(devices.begin(), devices.end()) > 0){
		//Sort Devices
		for (vector<tuple<Ipv4Address, uint16_t>>::iterator iterator = devices.begin(); // FOR device : devices
															iterator != devices.end(); 
															++iterator){
			//IF (!isPeerFull(device))
				connections.emplace(connections.end(), get<0>(*iterator)); // connectTo(device)
				cout << m_address << "\t" << get<0>(*iterator) << endl;
				//if (!(distance(connections.begin(), connections.end()) < min_peers))
				m_discovery->add(m_address, 20);
				break;
		}
	}else{
		if (i < limit*10){
			Simulator::Schedule(MilliSeconds(100), &p2p::FormationAux, this, ++i, limit);
		}else{
			setDiscoverable();
		}
	}
...
void p2p::setDiscoverable(void){
	m_discovery->add(m_address, 20); //setDiscoverable(true)
	//if (distance(devices.begin(), devices.end()) == 0){ //IF (peersCount() == 0)
	if (distance(connections.begin(), connections.end()) == 0){
		uint32_t timeout = 20000; //wait(timeout)
		Simulator::Schedule (MilliSeconds(timeout), &p2p::Formation, this); // goto start
	}
	//}
}

void p2p::DoWork(void){
	cout << Simulator::Now().GetMilliSeconds() << "ms\t" << m_address << endl;
	if (m_address != Ipv4Address("0.0.0.1")){
		Send("hi\nEND\n", Ipv4Address("0.0.0.1"), 20);
	}
}
...
void p2p::Setup(VirtualDiscovery *discovery, Ptr<UniformRandomVariable> rand)
{
...
// bool p2p::ReqFile(string fid, Ipv4Address reqAddress, uint32_t hops, bool toServer){
